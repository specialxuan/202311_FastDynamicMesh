...
void read_my_mode()
{
	...
	{
		if (i == 0)
			Message("%f \t", elas_mode[66654][i]);
		else
			Message("%12.10e \t", elas_mode[66654][i]);
	}
	Message("\n Validation is done! \n");
	Message("\n*******************************************\n");
}
...
DEFINE_GRID_MOTION(FDM_method, domain, dt, time, dtime)
{
	...
	real tmp_disp;                                 // temporary displacement
	...
	for (i = 0; i < 4; i++)
	{
		mode_force[time_index][i + 1] = 0;
	}
	memset(mode_force[time_index] + 1, 0, 4 * sizeof(real)); //TODO: test this command
	...
	begin_f_loop(f, t_fsi) // loop over all faces in a given face thread.
	{
		...
		f_node_loop(f, t_fsi, node_index) // loops over all nodes of a given face
		{
			...
			for (i = 1; i <= 4; i++)		     // calculation of the modal aerodynamic force
			{
		...
		begin_c_loop(c, t)
		{
			c_node_loop(c, t, node_index)
			{
				...
				if (NODE_POS_NEED_UPDATE(node))
				{
					...
					memset(disp, 0, 3 * sizeof(real)); // TODO: test this commend
										
					// calculation of nodal displacement for the current node
					for (i = 0; i < 4; i++)
					{
						tmp_disp = mode_disp[time_index][3 * i] - mode_disp[time_index - 1][3 * i];
						disp[0] += tmp_disp * N_UDMI(node, 3 * i);
						disp[1] += tmp_disp * N_UDMI(node, 3 * i + 1);
						disp[2] += tmp_disp * N_UDMI(node, 3 * i + 2);
					}
...
// ************************************************
// define_on_demand
// to output the calculated modal force,
// displacement and velocity to a txt file
// ************************************************
