...
int read_my_mode(double *elas_mode, const int row, const int column, const int i_Mode)
{
#if !RP_NODE
    FILE *fpInput = NULL;       // file pointer
    char inFileName[20] = {0}; // input file name
    if (i_Mode == 0)
        sprintf(inFileName, "NodeCoor.csv");
    else
        sprintf(inFileName, "NodeDisp%d.csv", i_Mode);
...
DEFINE_ON_DEMAND(Preprocess)
{
    ...
    else
    {
        ...
        // {
        //     if (fileStatus = read_my_mode(elas_mode, row, column, i_Mode))
        //     {
        //         Message(" Error: %d\n", i_Mode);
        //     }
        // }
